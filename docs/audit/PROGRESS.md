...
- WS8: `src/app/requestSecurity.ts`, `src/app/api/sameOrigin.ts`, `src/pipeline/jobSchedulerBootstrap.ts`, `src/app/api/jobs/resume` (new), `src/config/env.ts` (`THESIS_RESUME_ON_START` block only), `scripts/settings-reset.mjs` (new), `docs/PRIVACY.md` and `docs/DATA-RIGHTS.md` (new; README sections in WS9 link them), tests.
